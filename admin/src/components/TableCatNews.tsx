...
const TableNews = () => {
  const { items, deleteItem } = useCrud('news')
        ...
        <tbody>
          ...
          {itemsPaginated.map((item: any) => (
            <tr key={item._id}>
              ...
              <td data-label="Title" className="lg:w-1 whitespace-wrap">
                {item.title}
              </td>
              ...
              <td data-label="Text" className="lg:w-1 whitespace-wrap">
                {item.text}
              </td>
              <td className="before:hidden lg:w-1 whitespace-nowrap">
                <BaseButtons type="justify-start lg:justify-end" noWrap>
                  <BaseButton
                    color="info"
                    icon={mdiEye}
                    onClick={() => router.push(`/news?_id=${item._id}`)}
                    small
                  />
                  <BaseButton
                    color="danger"
