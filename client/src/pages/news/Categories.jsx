      ...
      <Combobox value={selected} onChange={setSelected}>
        <div className="relative mt-1">
          <div className="relative w-full cursor-default overflow-hidden rounded-lg bg-white dark:bg-teal-800 dark:border-white border text-left shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-teal-300 sm:text-sm">
            <Combobox.Input
              className="w-full border-none py-2 pl-3 pr-10 text-sm leading-5 text-gray-900 dark:text-white focus:ring-0 dark:bg-dark"
                ...
                filteredItems.map((item) => {
                  if (item.id !== selected.id) {
                    return (
                      <Combobox.Option
                        key={item.id}
                        className={({ active }) => ` relative cursor-default select-none py-2 pl-10 pr-4 ${active ? 'bg-teal-600 text-light dark:bg-light dark:text-gray-950' : 'dark:text-light dark:bg-dark text-gray-950 bg-light'}`}
                        value={item}
                      >
                        {({ selected, active }) => (
                          <>
